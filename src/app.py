...
with st.sidebar:
    ...
    if not os.path.exists(wpt_root):
        ...
        if st.button("🛠️ 自动初始化 WPT 环境", use_container_width=True):
            with st.status("正在初始化官方 WPT 环境...") as status:
                try:
        ...
        if server_manager.start(wpt_root):
            ...
            st.error("🔴 Server 端口占用")

    st.divider()
    st.markdown("#### 🌗 界面外观")
    is_dark = st.toggle("开启深色模式", value=(st.session_state.theme == 'dark'))
    st.session_state.theme = 'dark' if is_dark else 'light'

    st.divider()
    with st.expander("🔑 访问凭据 (API Keys)", expanded=True):
